...
#
#     # def is_duplicate_transaction(self, user_id, amount, timestamp):
#     #     """Check for duplicates within a 1-minute window around the timestamp"""
#     #     time_threshold_min = timestamp - timedelta(minutes=1)
#     #     time_threshold_max = timestamp + timedelta(minutes=1)
#     #
#     #     query = {
#     #         "user_id": user_id,
#     #         "amount": float(amount),
#     #         "timestamp": {
#     #             "$gte": time_threshold_min,
#     #             "$lte": time_threshold_max
#     #         },
#     #         "$or": [{"type": "request"}, {"type": "receipt"}]
#     #     }
#     #     return bool(self.transactions.find_one(query))
#
#     def is_duplicate_transaction(self, user_id, timestamp):
#         """Check for duplicates using exact timestamp only"""
#         query = {
#             "user_id": user_id,
#             "timestamp": timestamp,
#             "$or": [{"type": "request"}, {"type": "receipt"}]
...
class LoRaSerialMonitor:
    ...
    # def process_payment_request(self, message, node_address):
    #     """Handle payment requests with enhanced duplicate check and error handling"""
    #     try:
    #         payload = message.split("REQUEST_AMOUNT", 1)[1].strip()
    #         parts = payload.split()
    #         if len(parts) < 4:
    ...
    #             self.log_message(f"Payment error: {str(e)}\n", "error")
    #             self.send_message_to_node(node_address, f"TRX_ERROR|{str(e)}")
    #             # Remove failed request to allow retries
    ...
    # def process_payment_request(self, message, node_address):
    #     """Handle payment requests with exact timestamp duplicate check"""
    #     try:
    #         payload = message.split("REQUEST_AMOUNT", 1)[1].strip()
    #         parts = payload.split()
    #
    ...
    #             return
    #
    #         # Check for duplicate using exact timestamp
    #         if self.mongo_handler.is_duplicate_transaction(user_id, timestamp):
    #             self.log_message(f"Duplicate transaction prevented: {user_id} at {timestamp}\n", "error")
    #             self.send_message_to_node(node_address, "TRX_DUPLICATE")
    #             return
    #
    #         # Record request
    ...
    # def process_payment_request(self, message, node_address):
    #     """Handle payment requests with seconds-precision dedup + recent-window protection"""
    #     try:
    #         payload = message.split("REQUEST_AMOUNT", 1)[1].strip()
    #         parts = payload.split()
    #
    ...
    #             return
    #
    #         # 1) Quick CSV duplicate check (compare timestamps as strings truncated to seconds)
    #         ts_str = timestamp.strftime("%Y-%m-%d %H:%M:%S")
    #         if self.is_duplicate_request(user_id, ts_str, amount):
    #             self.log_message(f"Duplicate request (CSV) prevented: {user_id}, ₹{amount_float} at {ts_str}\n",
    #                              "error")
    #             self.send_message_to_node(node_address, "TRX_DUPLICATE")
    #             return
    #
    #         # 2) Recent-window check in MongoDB (configurable window_seconds)
    #         WINDOW_SECONDS = 60  # <-- tune as needed (e.g., 2 for immediate duplicate, 60 for retries)
    #         if self.mongo_handler.has_recent_request(user_id, amount_float, window_seconds=WINDOW_SECONDS):
    #             self.log_message(f"Duplicate request (recent-window) prevented: {user_id}, ₹{amount_float}\n", "error")
    #             self.send_message_to_node(node_address, "TRX_DUPLICATE")
    #             return
    #
    #         # 3) Record request to MongoDB (timestamp truncated inside handler)
    #         try:
    #             self.mongo_handler.record_request(node_address, user_id, name, amount_float, timestamp)
    #         except pymongo.errors.DuplicateKeyError:
    #             # This means the transaction already exists (same user_id + amount + timestamp (seconds) + type)
    #             self.log_message(f"Duplicate request (index) prevented: {user_id}, ₹{amount_float}\n", "error")
    #             self.send_message_to_node(node_address, "TRX_DUPLICATE")
    #             return
    #         except Exception as e:
    #             self.log_message(f"MongoDB error recording request: {str(e)}\n", "error")
    #             return
    #
    #         # 4) Process payment
    #         try:
    ...
    #         except Exception as e:
    #             self.log_message(f"Payment error: {str(e)}\n", "error")
    #             # You may want to remove the request doc to allow retry; be careful with race conditions.
    #             try:
    #                 self.mongo_handler.transactions.delete_many({
    #                     "user_id": user_id,
    #                     "amount": amount_float,
